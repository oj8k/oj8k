...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/oj8k/oj8k/raw/master/55r/shadowsocksr-manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/oj8k/oj8k/master/55r/ssrmu_centos -O /etc/init.d/ssrmu; then
